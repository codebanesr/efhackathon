...
export class UploadProviderAws extends UploadProvider {
  private static isGassInitialised: boolean = false
  ...
  public async initialise() {
    ...
    try {
      this.gasApiKey = process.env.SERVER_UPLOAD_MARBLISM_API_KEY

      if (Utility.isDefined(this.gasApiKey)) {
        if (UploadProviderAws.isGassInitialised) {
          return
        }

        await this.initializeWithGass()

        console.log(`AWS library (Gass) active in region ${this.region}`)

        UploadProviderAws.isGassInitialised = true

        return
      }
    } catch (error) {
      console.warn(`AWS library (Gass) failed to start: ${error.message}`)
    }
  ...
  private async initializeWithGass() {
    const url = `/v1/addons/upload/create-credentials`

    this.setApiKey(this.gasApiKey)

    const response = await this.postGass<CredentialsResponse>(url)
  ...
  private async postGass<ReturnType>(url: string) {
